 ...
 * html-excel-loader
 *
 * Automatically loads <excel src="..."> elements.
...
const EXCEL_SELECTOR = "excel";
const EXCEL_SOURCE_SELECTOR = "excel[src]";
...
function buildMergeIndex(merges) {
    ...
    merges.forEach((merge) => {
        ...
        if (owners.has(ownerKey) || covered.has(ownerKey)) {
            return;
        }
...
function renderWorksheet(worksheet) {
    ...
    const range = getRenderRange(worksheet, xlsx, merges);
    ...
    for (let row = range.s.r; row <= range.e.r; row += 1) {
        ...
        for (
            ...
            td.innerHTML = getDisplayValue(
                getCell(worksheet, row, column, xlsx),
                xlsx
            );
...
function renderWorkbook(workbook, container) {
    ...
    container.dispatchEvent(new CustomEvent("excel-loader:loaded", {
        detail: {
            workbook,
            worksheet,
            sheetName,
...
function reportLoadError(error, url, container) {
    console.error(`[html-excel-loader] 加载失败：${url}`, error);

    if (container) {
        container.dispatchEvent(new CustomEvent("excel-loader:error", {
            detail: { error, url }
        }));
    }
}
...
function loadExcelElement(element, force) {
    if (!element || element.tagName.toLowerCase() !== "excel") {
    ...
    if (!force && current && current.src === src) {
    ...
    element.setAttribute("data-excel-state", "loading");
    ...
    state.promise = readWorkbookFromRemoteFile(
        ...
        .then((workbook) => {
            if (excelElementLoads.get(element) !== state) {
                return null;
            }

            state.status = "loaded";
            element.setAttribute("data-excel-state", "loaded");
            return renderWorkbook(workbook, element);
        })
        .catch((error) => {
            ...
            if (excelElementLoads.get(element) === state) {
                state.status = "error";
                element.setAttribute("data-excel-state", "error");
                reportLoadError(error, src, element);
            }
...
function visitExcelElements(root, callback) {
    ...
    root.querySelectorAll(EXCEL_SELECTOR).forEach(callback);
}
...
function scanExcelElements(root) {
    ...
    scope
        .querySelectorAll(EXCEL_SOURCE_SELECTOR)
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === "attributes") {
                loadExcelElement(mutation.target, true);
                return;
            }

            mutation.addedNodes.forEach((node) => {
                visitExcelElements(node, loadExcelElement);
            });

            mutation.removedNodes.forEach((node) => {
                visitExcelElements(node, abortExcelElementLoad);
            });
        });
    });

...
# 烹饪系统
在据点的厨房中可以进行烹饪，也可以在野外区域从哥哥提尔德·柯涅尔处借用烹饪锅来获得仅限一次的烹饪机会（击破任意战斗点后刷新）。
...
<div class="table-container"><excel src="/zh_cn/assets/excel/料理与烹饪/料理与烹饪-1.xlsx"></excel></div>
...
<div class="table-container"><excel src="/zh_cn/assets/excel/料理与烹饪/料理与烹饪-2.xlsx"></excel></div>

</details>

料理配方中标 * 的食材 / 素材需要且仅能放置其中一种。

烹饪成品的品质等级由使用食材的品质等级决定。食材的品质越高，成品的品质就越高，效果也越显著。料理成品品质等级的计算方法为：$品质等级=\frac{加入的食材品质等级之和}{加入的食材总数}$，以 0 ~ 4 分别对应 ✦ ~ ✦✦✦✦✦ ，四舍六入五成双取整。<!--数据来源：RCP.Recipe.createDish() -> RCP.RecipeDish.calced_grade-->若料理配方中包含了其他料理成品，则该料理成品整体被视为 1 个食材，其品质等级单独计算。儿童用烹饪锅、虹吸壶和煤炭不被视为食材因而不参与计算。

除饮品和干粮外，料理无法在战斗中食用，且如果没有放进便当盒将会在战斗结束时腐败。饮品制作成功后需要空瓶子才能携带，但是可以在战斗中饮用，也不会腐败。干粮作为一种可堆叠的特殊料理，可以在战斗中食用，且即使不装入便当盒中也不会在战斗结束时腐败。
...
最大HP与最大MP由`基础数值`（即诺艾儿初期拥有的数值）和`技能数值`（即诺艾儿通过技能宝箱后期获得的数值）加和得到。最大HP的基础数值为150，数值下限为1；最大MP的基础数值为200，数值下限为15。在此基础上，最大HP与最大MP受料理效果影响的计算方法为：
...
料理的有效时间会在战斗结束时减少。此外，香草茶和能量饮料的效果会在高潮或失禁时减弱，其有效时间也会同时减少；鸡尾饮料会在高潮或失禁时完全消耗。料理效果的减弱与有效时间的减少的计算规则如下：

<!--数据来源：Stomach.progress()-->
...
此外，香草茶和能量饮料的有效时间（即饱腹度）也会因高潮或失禁而减少，其计算规则沿用战斗结束时的计算规则，但使用`失禁程度`替换`战斗点危险度`，且无视腹中非饮品料理的存在。`失禁程度`的计算规则为：

```c
失禁程度 = min(尿意, 50) * 0.05 + (50 - min(尿意, 50)) * 0.08
```

高潮或失禁时香草茶和能量饮料的效果值也会减少损失饱腹度的百分比：

```c
效果值 = 效果值 * (失禁后饱腹度 / 失禁前饱腹度)
```

不同于香草茶和能量饮料，鸡尾饮料在高潮或失禁时虽然与它们一同参与计算，但会在计算结束后完全消耗。
